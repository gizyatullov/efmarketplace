...
class PriceService:
    async def _get_price_with_binance(self,
                                      query: schemas.ReadPriceQuery = schemas.ReadPriceQuery()
    ...
    async def _set_price_in_redis(self,
                                  query: List[schemas.UnitPrice],
                                  ) -> None:
        async with Redis().from_url(url=str(settings.redis_url)) as r:
            for item in query:
                await r.set(name=f'{item.prefix_in_redis}{item.name_pair}',
                            value=item.price)
